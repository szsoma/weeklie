...
export default function TaskRow({ task }: Props) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`group relative flex items-start gap-3 py-2.5 pr-1.5 text-base leading-snug cursor-grab ${
    ...
    >
      <button
        ...
        className="w-4 h-4 mt-[3px] rounded-full flex-shrink-0 transition-transform hover:scale-110"
        style={{
      ...
      {isEditing ? (
        <input
          ...
          className="flex-1 bg-transparent border-b border-ink/30 outline-none text-base leading-snug py-0.5"
        />
      ) : (
        <span
      ...
      <label className="relative flex-shrink-0 inline-flex w-5 h-5 mt-[1px] cursor-pointer">
        ...
        />
        <span className="absolute inset-0 rounded-[6px] border border-rule-strong bg-bg transition-colors peer-checked:border-ink peer-checked:bg-ink" />
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="var(--bg)"
          ...
          className="pointer-events-none relative z-10 m-auto w-3 h-3 opacity-0 peer-checked:opacity-100 transition-opacity"
        >
      ...
      */}
      <button
        ...
        className="flex-shrink-0 w-5 mt-[1px] grid place-items-center text-faint hover:text-red-500 opacity-100 md:opacity-0 md:group-hover:opacity-100 focus:opacity-100 transition leading-none text-xl"
      >
        ×
      </button>
    </div>
